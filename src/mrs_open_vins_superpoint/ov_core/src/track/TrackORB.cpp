...
void TrackORB::feed_new_camera(const CameraData &message) {
...
void TrackORB::feed_monocular(const CameraData &message, size_t msg_id) {
...
void TrackORB::feed_stereo(const CameraData &message, size_t msg_id_left, size_t msg_id_right) {
  ...
  if (pts_last[cam_id_left].empty() || pts_last[cam_id_right].empty()) {
    ...
    std::vector<cv::KeyPoint> raw_left, raw_right;
    ...
    perform_detection_stereo(img_left, img_right, mask_left, mask_right, good_left, good_right, good_desc_left, good_desc_right,
                             cam_id_left, cam_id_right, good_ids_left, good_ids_right, &raw_left, &raw_right);
    std::lock_guard<std::mutex> lckv(mtx_last_vars);
    ...
    pts_last_raw[cam_id_left] = raw_left;
    pts_last_raw[cam_id_right] = raw_right;
  ...
  std::vector<cv::KeyPoint> pts_left_raw, pts_right_raw;
  ...
  perform_detection_stereo(img_left, img_right, mask_left, mask_right, pts_left_new, pts_right_new, desc_left_new, desc_right_new,
                           cam_id_left, cam_id_right, ids_left_new, ids_right_new, &pts_left_raw, &pts_right_raw);
  rT2 = boost::posix_time::microsec_clock::local_time();
  ...
  // Move forward in time
  {
    ...
    pts_last[cam_id_left] = good_left;
    pts_last[cam_id_right] = good_right;
    pts_last_raw[cam_id_left] = pts_left_raw;
    pts_last_raw[cam_id_right] = pts_right_raw;
    ids_last[cam_id_left] = good_ids_left;
    ids_last[cam_id_right] = good_ids_right;
    desc_last[cam_id_left] = good_desc_left;
    desc_last[cam_id_right] = good_desc_right;
...
void TrackORB::perform_detection_monocular(const cv::Mat &img0, const cv::Mat &mask0, std::vector<cv::KeyPoint> &pts0,
  ...
  const bool use_grid = (min_px_dist > 0);
  cv::Size size(1, 1);
  cv::Mat grid_2d;
  if (use_grid) {
    size = cv::Size((int)((float)img0.cols / (float)min_px_dist), (int)((float)img0.rows / (float)min_px_dist));
    grid_2d = cv::Mat::zeros(size, CV_8UC1);
  }
  ...
  for (size_t i = 0; i < pts0_ext.size(); i++) {
    ...
    int y_grid = 0;
    if (use_grid) {
      x_grid = (int)(kpt.pt.x / (float)min_px_dist);
      y_grid = (int)(kpt.pt.y / (float)min_px_dist);
...
void TrackORB::perform_detection_stereo(const cv::Mat &img0, const cv::Mat &img1, const cv::Mat &mask0, const cv::Mat &mask1,
                                        ...
                                        std::vector<size_t> &ids1, std::vector<cv::KeyPoint> *pts0_raw,
                                        std::vector<cv::KeyPoint> *pts1_raw) {
  ...
  // This means that we will reject points that less then grid_px_size points away then existing features
  const bool use_grid = (min_px_dist > 0);
  cv::Size size0(1, 1), size1(1, 1);
  cv::Mat grid_2d_0, grid_2d_1;
  if (use_grid) {
    size0 = cv::Size((int)((float)img0.cols / (float)min_px_dist), (int)((float)img0.rows / (float)min_px_dist));
    grid_2d_0 = cv::Mat::zeros(size0, CV_8UC1);
    size1 = cv::Size((int)((float)img1.cols / (float)min_px_dist), (int)((float)img1.rows / (float)min_px_dist));
    grid_2d_1 = cv::Mat::zeros(size1, CV_8UC1);
  }
  ...
  for (size_t i = 0; i < matches.size(); i++) {
    ...
    if (x0 < 0 || x0 >= img0.cols || y0 < 0 || y0 >= img0.rows) {
      continue;
    }
    int x1 = (int)kpt1.pt.x;
    int y1 = (int)kpt1.pt.y;
    if (x1 < 0 || x1 >= img0.cols || y1 < 0 || y1 >= img0.rows) {
      continue;
    }
    int x0_grid = 0;
    int y0_grid = 0;
    ...
    int y1_grid = 0;
    if (use_grid) {
      x0_grid = (int)(kpt0.pt.x / (float)min_px_dist);
      y0_grid = (int)(kpt0.pt.y / (float)min_px_dist);
      x1_grid = (int)(kpt1.pt.x / (float)min_px_dist);
      y1_grid = (int)(kpt1.pt.y / (float)min_px_dist);
      if (x0_grid < 0 || x0_grid >= size0.width || y0_grid < 0 || y0_grid >= size0.height || x1_grid < 0 || x1_grid >= size1.width ||
          y1_grid < 0 || y1_grid >= size1.height) {
        continue;
      }
    }

    // Check if this keypoint is near another point
    if (use_grid && (grid_2d_0.at<uint8_t>(y0_grid, x0_grid) > 127 || grid_2d_1.at<uint8_t>(y1_grid, x1_grid) > 127))
      continue;
    ...
    ids1.push_back(temp);
    if (use_grid) {
      grid_2d_0.at<uint8_t>(y0_grid, x0_grid) = 255;
      grid_2d_1.at<uint8_t>(y1_grid, x1_grid) = 255;
    }
  }
}

void TrackORB::robust_match(const std::vector<cv::KeyPoint> &pts0, const std::vector<cv::KeyPoint> &pts1, const cv::Mat &desc0,
...
void TrackORB::robust_ratio_test(std::vector<std::vector<cv::DMatch>> &matches) {
...
void TrackORB::robust_symmetry_test(std::vector<std::vector<cv::DMatch>> &matches1, std::vector<std::vector<cv::DMatch>> &matches2,
